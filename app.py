...
def kuhltemperatur_fur_entfeuchtung(ziel_abs_feuchte, sicherheit=1):
    taupunkt = 243.5 * np.log((ziel_abs_feuchte * 101325 / (0.622 + ziel_abs_feuchte))/611.2) / (17.67 - np.log((ziel_abs_feuchte * 101325 / (0.622 + ziel_abs_feuchte))/611.2)) if ziel_abs_feuchte > 0 else -50
...
def berechne_luftzustand(temp, rel_feuchte=None, abs_feuchte=None):
    ...
    return {'temperatur': round(temp, 1), 'rel_feuchte': round(rel_feuchte, 1) if rel_feuchte else 0, 'abs_feuchte': round(abs_feuchte * 1000, 2), 'enthalpie': round(enthalpie_feuchte_luft(temp, abs_feuchte) / 1000, 1)}
...
if st.button("📄 Professional PDF Report", type="primary"):
    ...
    daten = [f"Volumenstrom: {volumenstrom_final:,} m³/h", f"SFP (korrigiert): {sfp_final:.1f} W/(m³/h)", f"Betriebsmodus: {betriebsmodus}", f"WRG: {'Ja (' + str(wrg_wirkungsgrad) + '%)' if wrg_aktiv else 'Nein'}", f"Außenluft: {temp_aussen:.1f}°C, {feuchte_aussen}% rF", f"Zuluft: {temp_zuluft:.1f}°C" + (f", {feuchte_zuluft_soll:.1f}% rF" if betriebsmodus == 'Entfeuchten' else ""), "", "Leistungen:", f"  Ventilator: {p_ventilator:.1f} kW", f"  Heizung: {p_heizen_gesamt:.1f} kW", f"  Kühlung: {p_kuehlen_gesamt:.1f} kW", "", "Jahreskosten:", f"  Ventilator: {kosten_ventilator:,.0f} €/Jahr", f"  Heizung: {kosten_heizen:,.0f} €/Jahr", f"  Kühlung: {kosten_kuehlen:,.0f} €/Jahr", "", f"GESAMTKOSTEN: {gesamtkosten:,.0f} €/Jahr", f"Betriebsstunden: {gesamt_aktiv:,.0f} h/Jahr"]for item in daten:
    ...
    st.download_button(label="📥 PDF herunterladen", data=buffer, file_name=f"RLT_Professional_v6.2_{pd.Timestamp.now().strftime('%Y%m%d_%H%M')}.pdf", mime="application/pdf")
